...
public class AccountApis {
  ...
  public static void loginAsync(Context context, String account, String name,
    ...
    String requestUrl =
        String.format("http://a.brixd.com/conference/remote?method=user.login&account=%s", account);
    if (!TextUtils.isEmpty(name)) {
      requestUrl = requestUrl + "&name=" + name;
    }

    RestClientFactory.getClient().getAsync(
        new ContextAwareAPIDelegate<LoginResponse>(context, LoginResponse.class) {

          @Override
          public void onError(Throwable arg0) {
            if (callback != null) {
              callback.onLoginFailed(getContextProvider().getContext().getResources()
                  .getString(R.string.login_failed_caused_by_network));
            }
